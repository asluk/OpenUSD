...
def test_narrative_preserves_authored_argument_and_derives_summaries(narrative_case):
    ...
    assert len(deck["slides"]) == 8 and sum("image" in s for s in deck["slides"]) == 3
    assert any("Not built." in line for s in deck["slides"] for line in s["lines"])
    body = pr_body(updated, "test-branch")
    assert "Non-Hydra" in body and "Hydra" in body and "Not built." in body
...
def test_figures_are_bound_to_run_and_bytes(tmp_path, narrative_case):
    ...
    names = ("railway.png", "interpolation.png", "field.png")
    for name in names:
